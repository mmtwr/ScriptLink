...
namespace Rosecrance.ScriptLinkService.Data.Repositories.Odbc
{
    ...
    {
        public string GetCWSUserDefinedDictionaryValue(string facility, string fieldNumber, string dictionaryCode)
        {
            ...
            using (OdbcConnection connection = new OdbcConnection(_connectionStringCollection.CWS))
            {
                ...
                {
                    logger.Error(ex, "GetCWSUserDefinedDictionaryValue: Could not connect to ODBC data source. See error message. Data Source: {systemDsn}. Error: {errorMessage}", _connectionStringCollection.CWS, ex.Message);
                    throw;
                }
                catch (Exception ex)
                {
                    logger.Error(ex, "GetCWSUserDefinedDictionaryValue: An unexpected error occurred. Error Type: {errorType}. Error: {errorMessage}", ex.GetType(), ex.Message);
                    throw;
                }
            }
            return stringResult;
